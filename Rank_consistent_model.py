...
def residual_conv_block(filters, stage, block, strides=(1, 1), attention=None, cut='pre'):
    ...
    def layer(input_tensor):
        ...
        x = tf.keras.layers.BatchNormalization(name=bn_name + '1', gamma_regularizer=tf.keras.regularizers.l2(0.000005),**bn_params)(input_tensor)
        ...
        if cut == 'pre':
            ...
            shortcut = tf.keras.layers.Conv2D(filters, (1, 1), name=sc_name, strides=strides, kernel_regularizer=tf.keras.regularizers.l2(0.000005), **conv_params)(x)
        else:
            raise ValueError('Cut type not in ["pre", "post"]')

        # continue with convolution layers
        x = tf.keras.layers.ZeroPadding2D(padding=(1, 1))(x)
        x = tf.keras.layers.Conv2D(filters, (3, 3), strides=strides, name=conv_name + '1', kernel_regularizer=tf.keras.regularizers.l2(0.000005), **conv_params)(x)

        x = tf.keras.layers.BatchNormalization(name=bn_name + '2',gamma_regularizer=tf.keras.regularizers.l2(0.000005), **bn_params)(x)
        x = tf.keras.layers.Activation('relu', name=relu_name + '2')(x)
        x = tf.keras.layers.ZeroPadding2D(padding=(1, 1))(x)
        x = tf.keras.layers.Conv2D(filters, (3, 3), name=conv_name + '2', kernel_regularizer=tf.keras.regularizers.l2(0.000005), **conv_params)(x)
...
def ResNet(model_params, input_shape=None, input_tensor=None, include_top=True,
    ...
    # resnet bottom
    x = tf.keras.layers.BatchNormalization(name='bn_data',gamma_regularizer=tf.keras.regularizers.l2(0.000005), **no_scale_bn_params)(img_input)
    x = tf.keras.layers.ZeroPadding2D(padding=(3, 3))(x)
    x = tf.keras.layers.Conv2D(init_filters, (7, 7), strides=(2, 2), name='conv0', kernel_regularizer=tf.keras.regularizers.l2(0.000005), **conv_params)(x)
    x = tf.keras.layers.BatchNormalization(name='bn0',gamma_regularizer=tf.keras.regularizers.l2(0.000005), **bn_params)(x)
    ...
    x = tf.keras.layers.BatchNormalization(name='bn1',gamma_regularizer=tf.keras.regularizers.l2(0.000005), **bn_params)(x)
